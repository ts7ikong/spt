...
//			}
    ...
    @Override
    public void xzyqyhxxToUser(YqjbxxDTO yqjbxxDTO) {
        //根据园区Code创建用户信息
        SysUser user = new SysUser();
        user.setUsername(yqjbxxDTO.getParkCode());
        user.setRealname(yqjbxxDTO.getParkName());
        String salt = oConvertUtils.randomGen(8);
        user.setSalt(salt);
        //园区数据id作为密钥
        user.setPassword(yqjbxxDTO.getUuid());
        ...
        user.setWorkNo(yqjbxxDTO.getParkCode());
        user.setUserIdentity(1);
        user.setEmail(yqjbxxDTO.getParkName() + "@zagyspt.com");
        user.setPhone(yqjbxxDTO.getManangerPhone());
        user.setActivitiSync(1);
        //租户id为写死的 市平台-数据中心	1000
        //设置默认部门id 中安国研 1895411434184630274
        //设置默认角色id 市平台-园区角色 1906990917538672642
        sysUserService.saveUser(user, "", "1895411434184630274", "1000");
    }

    @Override
    public void scyqyhxxToUser(YqjbxxDTO yqjbxxDTO) {
        //根据园区Code创建用户信息
        SysUser user = new SysUser();
        sysUserService.deleteUserByUserName(yqjbxxDTO.getParkCode());
    }
