...
import org.jeecg.modules.system.model.SysLoginAuth;
import org.jeecg.modules.system.model.SysLoginAuthToQlt;
import org.jeecg.modules.system.model.SysLoginModel;
import org.jeecg.modules.system.service.*;
import org.jeecg.modules.system.service.impl.SysBaseApiImpl;
import org.jeecg.modules.system.util.HttpsaUtil;
import org.jeecg.modules.system.util.RandImageUtil;
import org.jeecg.modules.system.vo.tenant.TokenDto;
import org.jeecg.modules.system.vo.tenant.TokenDto.UploadTokenLogDto.UploadTokenLogsBackDto;
import org.jeecg.modules.system.vo.tenant.TokenDto.UploadTokenLogDto.UploadTokenParamDto;
import org.jeecg.modules.system.vo.tenant.TokenDto.UploadTokenLogDto.UploadTokenParamsDto;
...
public class LoginController {
    ...
    @Value("${qlt.rydw.username:rydw}")
    private String RYDWUSERNAME;

    @Value("${qlt.tszyp.username:tszyp}")
    private String TSZYPUSERNAME;

    @Value("${qlt.password:Zagy#135}")
    private String PWD;
     ...
     * @throws Exception
     */
    @RequestMapping(value = "/mAuth", method = RequestMethod.POST)
    public Result<JSONObject> mAuth(@RequestBody SysLoginModel sysLoginModel) throws Exception {
        Result<JSONObject> result = new Result<JSONObject>();
        String username = sysLoginModel.getUsername();
        String password = sysLoginModel.getPassword();
        JSONObject obj = new JSONObject();
        ...
        result.setSuccess(true);
        result.setCode(200);
        baseCommonService.addLog("用户名: " + username + ",鉴权成功！", CommonConstant.LOG_TYPE_1, null);
        return result;
    }


    /**
     * 根据园区编码和园区密钥鉴权（获取token）
     *
     * @param sysLoginAuth
     * @return
     * @throws Exception
    ...
    @ApiOperation(value = "根据园区编码和园区密钥鉴权", notes = "根据园区编码和园区密钥鉴权")
    public Result<JSONObject> mLogin(@RequestBody SysLoginAuth sysLoginAuth) throws Exception {
        Result<JSONObject> result = new Result<JSONObject>();
        String username = sysLoginAuth.getYqbm();
        String password = sysLoginAuth.getYqmy();
...
//        obj.put("userInfo", sysUser);
        ...
        result.setSuccess(true);
        result.setCode(200);
        baseCommonService.addLog("用户名: " + username + ",鉴权成功[接口推送]！", CommonConstant.LOG_TYPE_1, null);
        return result;
    }
     ...
     * @return token
     */
    public String qltLogin(String username) {
        SysUser sysUser = sysUserService.getUserByName(username);
        //2. 校验用户名或密码是否正确
        String userpassword = PasswordUtil.encrypt(username, PWD, sysUser.getSalt());
        String syspassword = sysUser.getPassword();
        if (!syspassword.equals(userpassword)) {
            addLoginFailOvertimes(username);
            throw new JeecgBootException("用户名或密码错误");
        }
    ...
    @Autowired
    HttpsaUtil httpsaUtil;

    @RequestMapping(value = "/qlt/tszyp/transferToken", method = RequestMethod.POST)
    @ApiOperation(value = "全链条获取token", notes = "全链条获取token")
    public Result<T> tokenTszyp(@RequestBody UploadTokenParamDto uploadTokenParamDto) {
        //TODO 最好校验dto里面传来的token
        String result = null;
        try {
            result = httpsaUtil.tnHttpPostBody(httpPostJsonHeaders(uploadTokenParamDto),
                    url,uploadTokenParamDto);
//                    "http://219.153.113.35:8990/euip-gapi-enterprise/gucs-user/api/euip/user/findUserByToken", uploadTokenParamDto);
            JsonObject jsonObject = JsonParser.parseString(result).getAsJsonObject();
            JsonObject data = jsonObject.getAsJsonObject("data");
            JsonObject userOutDTO = data.getAsJsonObject("userOutDTO");

            if (userOutDTO.has("district") && !userOutDTO.get("district").isJsonNull()) {
                String token = qltLogin(TSZYPUSERNAME);
                // 这里改为双引号
                String fullToken = token + "&info={\"path\":\"/tszyaqgk/tszyhdsj\"}";
                return Result.QLT(fullToken);
            }
        } catch (Exception e) {
    ...
    @RequestMapping(value = "/qlt/rydw/transferToken", method = RequestMethod.POST)
    @ApiOperation(value = "全链条获取token", notes = "全链条获取token")
    public Result<T> tokenRydw(@RequestBody UploadTokenParamDto uploadTokenParamDto) {
        //TODO 最好校验dto里面传来的token
        String result = null;
        try {
            result = httpsaUtil.tnHttpPostBody(httpPostJsonHeaders(uploadTokenParamDto),
...
//                    "http://219.153.113.35:8990/euip-gapi-enterprise/gucs-user/api/euip/user/findUserByToken", uploadTokenParamDto);
            ...
            if (userOutDTO.has("district") && !userOutDTO.get("district").isJsonNull()) {
                String token = qltLogin(RYDWUSERNAME);
                // 这里改为双引号
                String fullToken = token + "&info={\"path\":\"/rydwsj/sbrydwjcsj\"}";
                return Result.QLT(fullToken);
            }
    ...
    public static Map<String, String> httpPostJsonHeaders(UploadTokenParamDto uploadTokenParamDto) {
        ...
        header.put("Token", uploadTokenParamDto.getToken());
